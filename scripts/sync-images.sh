#!/usr/bin/env bash
# Sync homepage images from https://www.midex-eg.org into the Next.js public folder.
set -euo pipefail
...
mkdir -p "$IMG"/{hero,events,services,products,partners,exclusive,clients,news,blog,brand}
...
# Logos (from live site header)
download "1756200635.png" "$IMG/brand/logo-dark.png" 2>/dev/null || true
...
echo "Done — $(find "$IMG" -type f | wc -l | tr -d ' ') images in public/images."
