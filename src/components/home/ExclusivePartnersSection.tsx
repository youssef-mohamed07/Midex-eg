        ...
        <div className="mx-auto grid max-w-2xl grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
          {exclusivePartners.map((partner, index) => (
            <RevealOnScroll key={partner.name} delay={index * 70}>
              <div className="flex h-[4.5rem] items-center justify-center rounded-xl border border-midex-line bg-white px-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-midex-mint/45 hover:shadow-md sm:h-20">
                <Image
                  src={partner.image}
                  alt={partner.name}
                  width={160}
